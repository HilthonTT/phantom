        ...
        if let Some(state_key) = state_key {
            ...
            if let Some(Ok(membership)) = content.membership.map(|m| m.deserialize()) {
                ...
                if membership == MembershipState::Invite {
                    // ruma leaves `signed` unparsed; a token we cannot read
                    // names no third-party-invite event to authenticate
                    // against.
    ...
    Incoming: Event + Send + Sync,
{
    ...
    */
    ...
    if room_version.special_case_aliases_auth {
        ...
        if incoming_event.event_type().to_string() == ROOM_ALIASES_TYPE {
...
fn valid_membership_change(
    ...
    let power_levels: RoomPowerLevelsEventContent = match &power_levels_event {
        ...
        // `RoomPowerLevelsEventContent` no longer implements `Default`; every
        // field defaults during deserialization, so an empty object gives the
        // spec's defaults without having to name a rule set.
        None => from_json_str("{}")?,
    };
...
fn verify_third_party_invite(
    ...
    // ruma hands `signed` over unparsed, so the mxid and token checks below
    // need it deserialized first.
    #[allow(clippy::manual_let_else)]
    ...
    // The public keys are base64 strings rather than decoded bytes now, so the
    // comparison has to happen on the decoded form of both sides.
