    ...
    T: for<'de> Deserialize<'de>,
{
    self.get_unsigned_as_value()
        ...
        .ok_or(err!(Request(NotFound(
            "property not found in unsigned object"
        ))))?
        .map_err(|e| {
            err!(Database(
                "Failed to deserialize unsigned.{property} into type: {e}"
            ))
        })
}
...
pub fn get_unsigned<T>(&self) -> Result<JsonValue> {
    self.unsigned
        ...
        .ok_or(err!(Request(NotFound(
            "\"unsigned\" property not found in pdu"
        ))))?
        .map_err(|e| {
            err!(Database(
                "Failed to deserialize \"unsigned\" into value: {e}"
            ))
        })
}
