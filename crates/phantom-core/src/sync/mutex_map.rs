...
mod tests {
    ...
    #[test]
    fn entries_are_reaped_when_the_last_guard_drops() {
        let map: MutexMap<String, ()> = MutexMap::new();
        assert!(map.is_empty());

        {
            let _guard = block_on(map.lock("foo"));
            assert_eq!(map.len(), 1);
            assert!(map.contains(&"foo".to_owned()));
        }

        assert!(map.is_empty(), "the entry outlived its only guard");
    }

    #[test]
    fn a_held_key_cannot_be_locked_twice() {
        let map: MutexMap<String, ()> = MutexMap::new();
