...
//!
//! `append_pdu`, `append_incoming_pdu` and `redact_pdu` were ported first and
//! live in [`append`](super::append). `create_hash_and_sign_event`,
//! `build_and_append_pdu` and `check_pdu_for_admin_room` followed them and
//! live in [`build`](super::build). What is left here is backfill:
//!
//! - `backfill_if_required` — deciding whether the history a client is asking
//!   for is short enough that another server should be asked for more
//! - `backfill_pdu` — taking one event from that answer and prepending it to
//!   the room
//!
//! Both wait on the porting itself rather than on any service: they reach for
//! conduwuit's names for the federation client and for an older ruma. The
//! imports the parked code needs are conduwuit's, not phantom's, and are left
//! alone for the same reason the bodies are:
//!
//! ```ignore
//! use conduwuit::{
