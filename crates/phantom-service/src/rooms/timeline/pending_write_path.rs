//! The timeline write path, waiting on the services it calls into.
//!
...
//! path as pasted, kept verbatim so it can be ported function by function as
//! its dependencies land, rather than rewritten from scratch later.
//!
//! What each part is waiting on:
//!
//! - `append_pdu`: `rooms::state`, `rooms::state_cache`, `rooms::search`,
//!   `rooms::pdu_metadata`, `rooms::read_receipt`, `rooms::threads`,
//!   `rooms::alias`, `rooms::spaces`, `admin`, `appservice`, `sending`,
//!   `pusher`, `users`, `account_data`
//! - `create_hash_and_sign_event`: `rooms::state`, `rooms::state_accessor`,
//!   `rooms::short`, `server_keys`
//! - `build_and_append_pdu`: `rooms::state`, `rooms::state_cache`,
//!   `rooms::state_accessor`, `admin`, `sending`
//! - `append_incoming_pdu`: `rooms::state`, `rooms::state_compressor`
//! - `redact_pdu`: `rooms::search`
//! - `backfill_if_required` / `backfill_pdu`: `rooms::state_cache`,
...
//! ```
//!
//! Along with the `ExtractRelatesTo` / `ExtractEventId` /
//! `ExtractRelatesToEventId` / `ExtractBody` deserializers, the
//! `RoomMutexMap` / `RoomMutexGuard` aliases and the `mutex_insert` field,
//! all of which only the write path uses.
    ...
    pub async fn redact_pdu(
        ...
        let obj = utils::to_canonical_object(&pdu).map_err(|e| {
            err!(Database(error!(
                ?event_id,
                ?e,
                "Failed to convert PDU to canonical JSON"
            )))
        })?;
