...
pub use phantom_core::matrix::pdu::{ShortEventId, ShortId, ShortRoomId, ShortStateKey};
use phantom_core::{
    ...
    I: Iterator<Item = &'a EventId> + Clone + Debug + Send + 'a,
{
    self.db
        ...
        .zip(event_ids.stream())
        .map(|(result, event_id)| match result {
            Ok(ref short) => u64_from_u8(short),
...
pub async fn get_or_create_shortstatekey(
    ...
    self.db.shortstatekey_statekey.put(shortstatekey, key).ok();
    ...
    <Id as ToOwned>::Owned: Borrow<EventId>,
{
    ...
    self.db
        ...
        .map_err(|e| {
            err!(Database(
                "Failed to find EventId from short {shorteventid:?}: {e:?}"
            ))
        })
}
