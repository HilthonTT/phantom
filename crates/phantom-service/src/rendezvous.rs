...
pub fn create(&self, data: Bytes) -> (SessionId, Meta) {
    let config = &self.services.config;
    let ttl = Duration::from_secs(config.rendezvous.rendezvous_session_ttl);

    self.create_at(
        data,
        SystemTime::now(),
        ttl,
        config.rendezvous.rendezvous_max_sessions,
    )
}
...
pub fn put(&self, id: &str, if_match: &str, data: Bytes) -> Put {
    let ttl = Duration::from_secs(self.services.config.rendezvous.rendezvous_session_ttl);
...
#[must_use]
#[inline]
pub fn sequence_token(&self) -> &str {
...
pub fn expires_in(&self) -> Duration {
    self.expires_in_at(SystemTime::now())
}
