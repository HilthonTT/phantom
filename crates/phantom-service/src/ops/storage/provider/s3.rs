...
pub(in super::super) fn new(
    ...
    if config.url.is_none() && config.bucket.is_none() {
        debug!(
            ?name,
            "s3_provider.bucket not set. This configuration will be skipped"
        );
        return Ok(None);
    }

    let network = &args.server.config.network;
    let mut builder = AmazonS3Builder::from_env().with_client_options(
        ClientOptions::new()
            .with_user_agent(user_agent().try_into()?)
            .with_pool_max_idle_per_host(network.request_idle_per_host.into())
            .with_pool_idle_timeout(Duration::from_secs(network.request_idle_timeout)),
    );
