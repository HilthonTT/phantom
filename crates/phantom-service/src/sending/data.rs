...
impl Data {
    ...
    pub(super) fn queue_requests<'a, I>(&self, requests: I) -> Result<Vec<Vec<u8>>>
    where
...
fn parse_servercurrentevent(key: &[u8], value: &[u8]) -> Result<(Destination, SendingEvent)> {
    ...
    Ok::<_, Error>(if key.starts_with(b"+") {
        ...
        let server = text::string_from_bytes(server).map_err(|_| {
        ...
        let user_string = text::str_from_bytes(user)
        ...
        let pushkey = parts
            .next()
            .ok_or_else(|| Error::bad_database("Invalid bytes in servercurrentpdus."))?;
        let pushkey_string = text::string_from_bytes(pushkey)
