...
use phantom_database::{Cbor, Deserialized, Txn, serialize_to_vec};
use ruma::thirdparty::Medium;
use serde::{Deserialize, Serialize};
use subtle::ConstantTimeEq;

use super::{Association, UiaaKey, claim_lock_key};
...
/// Session id stored as the value of a claim index row.
type ClaimSid = String;
...
pub async fn create_or_reuse_pending(
    ...
    let token = rand::string(TOKEN_LENGTH);
    ...
    Ok(PendingOutcome {
        sid,
        freshly_minted_token: Some(token),
    })
}
...
pub async fn claim_validated(
    ...
    let _claim_lock = self.claim_mutex.lock(&claim_lock_key(&claim)).await;
...
pub async fn redeem_claim(&self, claim: &UiaaKey) -> Result<Association> {
    ...
    if current_sid != sid {
        return Err!(Request(ThreepidAuthFailed(
            "The verification session claim has changed"
        )));
    }
...
pub async fn redeem_validated(&self, sid: &str, client_secret: &str) -> Result<Association> {
    ...
    if pending.validated_at.is_none() {
        return Err!(Request(ThreepidAuthFailed(
            "The address has not been validated"
        )));
    }
...
#[implement(super::Service)]
fn persist_pending(&self, sid: &str, pending: &Pending) -> Result {
    self.db.threepidsid_pending.raw_put(sid, Cbor(pending))
}

#[implement(super::Service)]
async fn delete_pending_state(&self, sid: &str, pending: &Pending) -> Result<()> {
    let PendingUse::Claimed(claim) = &pending.use_state else {
        return self.delete_pending_rows(sid, None);
    };
...
#[implement(super::Service)]
fn delete_pending_rows(&self, sid: &str, claim: Option<&UiaaKey>) -> Result {
    let mut txn = Txn::new(&self.db.database.engine);
    txn.remove(&self.db.threepidsid_pending, sid);
...
#[implement(super::Service)]
fn delete_claim_index(&self, claim: &UiaaKey) -> Result {
    self.db.userdevicesessionid_threepid.del(claim)
}

#[implement(super::Service)]
async fn claim_sid(&self, claim: &UiaaKey) -> Result<Option<ClaimSid>> {
    match self.db.userdevicesessionid_threepid.qry(claim).await {
...
fn expired(pending: &Pending) -> bool {
    pending.expires_at.is_some_and(timepoint_has_passed)
}
...
mod tests {
    ...
    fn legacy_pending_defaults_to_available() {
        ...
        let encoded = serialize_to_vec(Cbor(legacy)).expect("legacy pending row should serialize");
