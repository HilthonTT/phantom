...
use super::{
    ...
    fed::{DEFAULT_PORT_NUM, FedDest, PortString, add_port_to_hostname, get_ip_with_port},
};
...
impl super::Service {
    ...
    pub async fn lookup_actual_dest(&self, server_name: &ServerName) -> Result<(CachedDest, bool)> {
            ...
            None => {
                if let Some(pos) = delegated.find(':') {
                    ...
                    match self.query_srv_record(&delegated).await? {
                        Some(overrider) => self.actual_dest_3_3(cache, delegated, overrider).await,
                        None => self.actual_dest_3_4(cache, delegated).await,
                    }
                }
            }
        }
    ...
    async fn actual_dest_3_3(
        ...
        if let Some(port) = force_port {
            return Ok(FedDest::Named(delegated, port_string(port)));
        }
    ...
    async fn actual_dest_4(&self, host: &str, cache: bool, overrider: FedDest) -> Result<FedDest> {
        ...
        if let Some(port) = force_port {
            return Ok(FedDest::Named(host.to_owned(), port_string(port)));
        }
...
/// A port as it is stored on a [`FedDest::Named`], with the leading colon.
fn port_string(port: u16) -> PortString {
    PortString::from(format!(":{port}").as_str()).unwrap_or_else(|_| FedDest::default_port())
}
