...
use phantom_core::{Error, Result, checked, debug::type_name, err, result::DebugInspect, strings};
use serde::{
    Deserialize, de,
...
impl<'a, 'de: 'a> de::Deserializer<'de> for &'a mut Deserializer<'de> {
    ...
    fn deserialize_string<V: Visitor<'de>>(self, visitor: V) -> Result<V::Value> {
        let input = self.record_next();
        let out = strings::string_from_bytes(input)?;
